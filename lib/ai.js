...
async function generateCompletion({ prompt, temperature = 0.5, maxTokens = 1500, type = 'text', context = {} }) {
  ...
  logger.info('ai_provider_check', { hasOpenAI, hasAnthropic, hasGroq, hasHuggingFace });
  
  // Try providers sequentially: Groq → OpenAI → Template → Hugging Face
  
  // 1. Try Groq first (fastest, free tier generous)
  if (hasGroq) {
    try {
      const groq = getGroq();
      const opts = {
      ...
      if (content) {
        logger.info('groq_success', { provider: 'groq' });
        return { provider: 'groq', content };
      }
    } catch (err) {
      logger.warn('groq_failed', { message: err.message });
    }
  }
  ...
  if (hasOpenAI) {
    try {
      ...
      const content = resp.choices?.[0]?.message?.content?.trim();
      if (content) {
        logger.info('openai_success', { provider: 'openai' });
        return { provider: 'openai', content };
      }
    } catch (err) {
      logger.warn('openai_failed', { message: err.message });
    }
  }
  
  // 3. Template generation fallback (guaranteed to work)
  logger.info('ai_fallback_to_template', { type: context.type, availableProviders: { hasOpenAI, hasGroq, hasHuggingFace } });
  ...
  // 4. Last resort: Try Hugging Face
      ...
      if (process.env.HUGGINGFACE_API_KEY) {
        try {
          ...
          if (directResponse && directResponse[0]?.generated_text) {
            ...
            if (content) {
              logger.info('huggingface_direct_fallback_success', { provider: 'huggingface_direct' });
              return { provider: 'huggingface_direct', content };
            }
          }
        } catch (directErr) {
