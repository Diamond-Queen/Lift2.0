...
function buildResumeTemplate({ name, email, phone, address, linkedin, objective, experience, education, skills, certifications }) {
  ...
  const generateObjective = (obj) => {
    ...
    // Generic/catch-all: Build professional summary from the user's input
      ...
      // For short inputs, expand into full professional summary
      if (userInput.length < 50) {
        generatedSummary = `Accomplished professional with strong expertise in ${userInput.toLowerCase()}. Committed to delivering measurable results, continuous improvement, and making meaningful contributions to organizational success through collaboration and excellence.`;
      } else {
        // For longer inputs, weave key concepts into a cohesive summary
        const keyWords = userInput.match(/\b(problem|solution|lead|manage|build|create|drive|improve|strategic|innovation|growth|excellence)\w*/gi) || [];
        const focus = keyWords.length > 0 ? keyWords.slice(0, 2).join(' and ') : 'professional excellence';
        generatedSummary = `${userInput}. Dedicated to achieving ${focus}, driving organizational success, and making meaningful contributions through proven expertise, strategic thinking, and commitment to continuous improvement.`;
      }
    }
