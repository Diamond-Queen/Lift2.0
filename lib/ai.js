...
/**
 * SIMPLIFIED generation: Template first (instant) → Groq AI (fallback for better quality)
 * For flashcards, quizzes, resumes: Use fast templates, optionally upgrade with AI.
 * 
 * PERFORMANCE: <100ms average (templates instant). Groq only if requested/needed.
 */
async function generateCompletion({ prompt, temperature = 0.5, maxTokens = 1500, type = 'text', context = {} }) {
  // For flashcards, quizzes, and resumes: Use template first, Groq as enhancement
  if (['flashcards', 'quiz', 'resume'].includes(context.type)) {
    // Use template immediately (guaranteed fast)
    let templateContent;
    if (context.type === 'resume') {
      templateContent = JSON.stringify(buildResumeTemplate(context));
    } else if (context.type === 'flashcards') {
      templateContent = JSON.stringify(buildFlashcardsTemplate(context));
    } else if (context.type === 'quiz') {
      templateContent = JSON.stringify(buildQuizTemplate(context));
    }
    
    if (templateContent) {
      logger.info('ai_using_template_first', { type: context.type });
      
      // Try to enhance with Groq AI (optional improvement)
      // Quiz gets shorter timeout (3s) since it's lower priority and we have template fallback
      const timeoutMs = context.type === 'quiz' ? 3000 : 6000;
      const groq = getGroq();
      if (groq) {
        try {
          ...
          if (content) {
            ...
            if (context.type === 'json') {
              try {
                ...
                logger.warn('groq_invalid_json_enhancement', { message: jsonErr.message });
                // Use template instead
                return { provider: 'template', content: templateContent };
              }
            }
            
            logger.info('ai_enhanced_with_groq', { type: context.type });
            return { provider: 'groq', content };
          }
        } catch (err) {
          logger.warn('groq_enhancement_failed', { message: err.message, type: context.type });
  ...
  // For other types (summary, etc): Try Groq with 6-second timeout
  const aiTimeout = new Promise((_, reject) => 
  ...
  // Fallback to template for other types
  logger.info('ai_using_template_fallback', { type: context.type });
  
  if (context.type === 'summary') {
    return { provider: 'template', content: buildSummaryTemplate(context) };
  }

  throw new Error('No provider available for type: ' + context.type);
}
...
function buildQuizTemplate({ notes }) {
  ...
  // Helper to extract key terms/concepts for better question generation
  // Prioritizes actual useful information regardless of capitalization
  const extractKeyTerms = (str) => {
    const stopWords = new Set(['the', 'a', 'an', 'is', 'are', 'was', 'were', 'be', 'been', 'have', 'has', 'had', 'do', 'does', 'did', 'will', 'would', 'could', 'should', 'may', 'might', 'must', 'can', 'and', 'or', 'but', 'in', 'on', 'at', 'to', 'from', 'of', 'with', 'by', 'for', 'as', 'what', 'which', 'that', 'this', 'these', 'those', 'i', 'you', 'he', 'she', 'it', 'we', 'they', 'me', 'him', 'her', 'us', 'them']);
    const terms = [];
    
    // 1. Extract noun phrases (often the most useful info)
    // Look for adjective + noun patterns or multi-word noun phrases
    const nounPhrases = str.match(/\b[A-Z]?[a-z]+(?:\s+[A-Z]?[a-z]+)?(?:\s+(?:process|system|method|theory|concept|principle|law|rule|mechanism|function|property|characteristic|effect|result|outcome))?/gi) || [];
    terms.push(...nounPhrases.filter(p => {
      const cleaned = p.trim().toLowerCase();
      const words = cleaned.split(/\s+/);
      return p && p.length > 3 && p.length < 50 && !words.every(w => stopWords.has(w));
    }).slice(0, 3));
    
    // 2. Extract capitalized words (proper nouns, names, technical terms)
    const capitalizedWords = str.match(/\b[A-Z][a-z]+(?:\s+[A-Z][a-z]+)*/g) || [];
    terms.push(...capitalizedWords.filter(t => {
      const cleaned = t.toLowerCase();
      return t && t.length > 2 && !stopWords.has(cleaned);
    }).slice(0, 2));
    
    // 3. Extract domain-specific or technical terms
    const technicalPatterns = str.match(/\b(?:the\s+)?([a-z]+(?:\s+[a-z]+)?)\s+(?:of|in|during|at|through|via|by)\b/gi) || [];
    terms.push(...technicalPatterns.filter(t => {
      const cleaned = t.toLowerCase();
      const words = cleaned.split(/\s+/).filter(w => w && !stopWords.has(w));
      return t && t.length > 3 && words.length > 0;
    }).slice(0, 2));
    
    // 4. Extract phrases with verbs (often convey action/process - useful info)
    const verbPhrases = str.match(/\b[a-z]+s\s+(?:the\s+)?[a-z]+(?:\s+[a-z]+)?\b/gi) || [];
    terms.push(...verbPhrases.filter(p => {
      const cleaned = p.toLowerCase();
      const words = cleaned.split(/\s+/).filter(w => w && !stopWords.has(w));
      return p && p.length > 5 && words.length > 0;
    }).slice(0, 2));
    
    // Remove duplicates and filter, prioritize longer meaningful terms
    const unique = [...new Set(terms)]
      .filter(t => {
        const cleaned = t.trim().toLowerCase();
        const words = cleaned.split(/\s+/).filter(w => w && !stopWords.has(w));
        return t && t.trim().length > 2 && words.length > 0;
      })
  ...
  // Helper to build topic-aware distractors
  const makeDistractors = (answer, allCandidates, topic) => {
    const distractors = new Set();
    const a = String(answer).trim();
    
    // Try to find different sentences from candidates as distractors
    for (const cand of allCandidates) {
      const c = String(cand).trim();
      if (c !== a && c.length > 3 && c.length < 150 && !c.includes(a)) {
        const dist = c.slice(0, 100);
        distractors.add(dist);
        if (distractors.size >= 3) break;
      }
    }
    
    // If not enough distractors from candidates, create plausible variants
    if (distractors.size < 2 && answer.length > 10) {
      const words = answer.split(/\s+/).filter(Boolean);
      
      // Variant 1: partial answer (missing key detail)
      if (words.length > 2) {
        distractors.add(words.slice(0, -1).join(' '));
      }
      
      // Variant 2: related but incorrect alternative
      if (topic) {
        const alternatives = [
          `The opposite of ${topic}`,
          `A different aspect of ${topic}`,
          `Related to ${topic} but not directly applicable`
        ];
        distractors.add(alternatives[0]);
      } else {
  ...
  for (let i = 0; i < candidates.length && quiz.length < 10; i++) {
    ...
    // Heuristic 1: if line contains ':' treat as term:definition
    if (cleaned.includes(':')) {
      const [term, def] = cleaned.split(':').map(s => s.trim());
      if (term && def && term.length > 2 && def.length > 5) {
        question = `What is "${term}"?`;
        answer = def;
        topic = term;
      }
    } 
    // Heuristic 2: Extract key fact/concept from statement and make it topic-specific
    else {
      const keyTerms = extractKeyTerms(cleaned);
      const firstKeyTerm = keyTerms[0];
      
      // Only use key term if it's reasonably substantial
      if (firstKeyTerm && firstKeyTerm.trim().length > 3) {
        topic = firstKeyTerm;
        // Generate specific question about this topic
        const questionPatterns = [
          `What is the role of ${firstKeyTerm}?`,
          `Which statement about ${firstKeyTerm} is correct?`,
          `Based on the material, what describes ${firstKeyTerm}?`,
          `How does ${firstKeyTerm} relate to the concepts discussed?`
        ];
        question = questionPatterns[i % questionPatterns.length];
        answer = cleaned.slice(0, 120);
      } else {
        // Fallback: extract the most informative sentence or phrase
        // Prioritize sentences with verbs, numbers, or key descriptive words
        const sentences = cleaned.split(/[.!?]/).filter(s => s.trim().length > 10);
        let bestSentence = null;
        
        // Find the sentence with the most useful information markers
        if (sentences.length > 0) {
          bestSentence = sentences.reduce((best, current) => {
            const currentScore = (
              (current.match(/\b(?:is|are|has|have|causes|creates|results|leads|determines|affects|influences)\b/gi) || []).length * 2 +
              (current.match(/\d/g) || []).length +
              (current.length / 50) // longer informative sentences
            );
            const bestScore = (
              (best.match(/\b(?:is|are|has|have|causes|creates|results|leads|determines|affects|influences)\b/gi) || []).length * 2 +
              (best.match(/\d/g) || []).length +
              (best.length / 50)
            );
        ...
        if (bestSentence && bestSentence.trim().length > 10) {
          answer = bestSentence.trim();
          question = `Which of the following accurately describes this concept?`;
        }
      }
    }
    ...
    const distractors = makeDistractors(answer, candidates, topic);
    if (distractors.length < 2) continue; // Skip if we can't make good options
    
    // Build options: [distractors, answer] then shuffle
    const options = [...distractors.slice(0, 2), answer];
    ...
    if (correctIndex < 0) continue; // Skip if answer not in options (shouldn't happen)
    
    const correctOption = String.fromCharCode(65 + correctIndex); // A, B, C
...
function buildSummaryTemplate({ notes, summaryLength }) {
  ...
  const lengthConfig = {
    'short': { maxTopics: 2, detail: 'brief', elaborationLength: 1 },
    'medium': { maxTopics: 4, detail: 'moderate', elaborationLength: 2 },
    'long': { maxTopics: 7, detail: 'comprehensive', elaborationLength: 3 }
  };
  const config = lengthConfig[summaryLength] || lengthConfig['medium'];
  
  let elaboration = '';
  
  // Extract and elaborate on key topics based on summaryLength config
  const keyTopics = lines.slice(0, Math.min(config.maxTopics, lines.length));
  ...
  keyTopics.forEach((topic, idx) => {
    ...
    // Add contextual elaboration based on the topic and detail level
    let detailText = '';
    if (cleaned.toLowerCase().includes('photosynthesis') || cleaned.toLowerCase().includes('plant')) {
      detailText = 'This biological process is fundamental to life on Earth, converting light energy into chemical energy stored in glucose molecules. Plants, algae, and some bacteria use this process to produce their own food.';
    } else if (cleaned.toLowerCase().includes('formula') || cleaned.toLowerCase().includes('equation')) {
      detailText = 'Mathematical formulas provide precise relationships between variables, allowing us to predict outcomes and understand patterns. Understanding the meaning of each component is crucial for proper application.';
    } else if (cleaned.toLowerCase().includes('cycle') || cleaned.toLowerCase().includes('process')) {
      detailText = 'Sequential processes involve multiple interconnected steps where each stage depends on the previous one. Breaking down these steps helps in understanding the overall mechanism and identifying key transition points.';
    } else {
      detailText = `This concept represents an important foundation for understanding the broader topic. It connects to related ideas through cause-and-effect relationships and provides context for more advanced applications.`;
    }
    
    // Add elaboration sentences based on detail level
    if (config.elaborationLength === 1) {
      // Brief: first sentence only
      elaboration += detailText.split('.')[0] + '.\n\n';
    } else if (config.elaborationLength === 2) {
      // Moderate: main explanation
      elaboration += detailText + '\n\n';
    } else if (config.elaborationLength === 3) {
      // Comprehensive: full explanation with additional context
      elaboration += detailText + '\n\n';
      elaboration += `This is essential knowledge for deeper understanding and practical application of this concept.\n\n`;
    }
  });
