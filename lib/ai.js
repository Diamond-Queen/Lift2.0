...
function buildQuizTemplate({ notes }) {
  ...
  const extractKeyTerms = (str) => {
    // Look for capitalized words, numbers, technical terms
    const capitalizedWords = str.match(/\b[A-Z][a-z]+(?:\s+[A-Z][a-z]+)*/g) || [];
    const technicalTerms = str.match(/\b[a-z]+(?:-[a-z]+)?(?:\s+[a-z]+(?:-[a-z]+)?){0,2}\b/gi) || [];
    return [...new Set([...capitalizedWords, ...technicalTerms.slice(0, 3)])].filter(t => t && t.length > 2);
  };
  ...
  for (let i = 0; i < candidates.length && quiz.length < 10; i++) {
    ...
    // Heuristic 2: Extract key fact/concept from statement and make it topic-specific
      ...
      if (firstKeyTerm && firstKeyTerm.length > 2) {
        ...
        // Fallback: extract a meaningful statement
        const sentences = cleaned.split(/[.!?]/).filter(s => s.trim().length > 10);
        if (sentences.length > 0) {
          answer = sentences[0].trim();
          question = `Which of the following accurately describes the concept?`;
        }
      }
    }
