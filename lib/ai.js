...
async function generateCompletion({ prompt, temperature = 0.5, maxTokens = 1500, type = 'text', context = {} }) {
  ...
  if (['flashcards', 'quiz', 'resume'].includes(context.type)) {
    ...
    if (templateContent) {
      ...
      // Try to enhance with Groq AI (optional improvement, 15-second timeout for production)
      const groq = getGroq();
      if (groq) {
        try {
          const groqTimeout = process.env.NODE_ENV === 'production' ? 15000 : 6000;
          const aiTimeout = new Promise((_, reject) => 
            setTimeout(() => reject(new Error('AI timeout')), groqTimeout)
          );
  ...
  // For other types (summary, etc): Try Groq with 15-second timeout for production
  const groqTimeout = process.env.NODE_ENV === 'production' ? 15000 : 6000;
  const aiTimeout = new Promise((_, reject) => 
    setTimeout(() => reject(new Error('AI timeout')), groqTimeout)
  );
