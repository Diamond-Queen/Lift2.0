...
function buildQuizTemplate({ notes }) {
  ...
  const extractKeyTerms = (str) => {
    ...
    terms.push(...nounPhrases.filter(p => p && p.length > 3 && p.length < 50).slice(0, 3));
    
    // 2. Extract capitalized words (proper nouns, names, technical terms)
    const capitalizedWords = str.match(/\b[A-Z][a-z]+(?:\s+[A-Z][a-z]+)*/g) || [];
    terms.push(...capitalizedWords.filter(t => t && t.length > 2).slice(0, 2));
    
    // 3. Extract domain-specific or technical terms (words followed by specialized keywords)
    const technicalPatterns = str.match(/\b(?:the\s+)?([a-z]+(?:\s+[a-z]+)?)\s+(?:of|in|during|at|through|via|by)\b/gi) || [];
    terms.push(...technicalPatterns.filter(t => t && t.length > 3).slice(0, 2));
    ...
    terms.push(...verbPhrases.filter(p => p && p.length > 5).slice(0, 2));
    
    // Remove duplicates and filter empty/short strings, prioritize longer meaningful terms
  ...
  for (let i = 0; i < candidates.length && quiz.length < 10; i++) {
    ...
    // Heuristic 2: Extract key fact/concept from statement and make it topic-specific
      ...
      if (firstKeyTerm && firstKeyTerm.length > 2) {
        topic = firstKeyTerm;
