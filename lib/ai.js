...
/**
 * Generate completion with tiered fallback (optimized for speed):
 * 1. OpenAI gpt-4o-mini (with 8s timeout) - PRIMARY (fastest, most reliable)
 * 2. Anthropic Claude 3.5 Sonnet (with 8s timeout) - PARALLEL with OpenAI
 * 3. Hugging Face Inference API (with 5s timeout) - FALLBACK (free but slow)
 * 4. Enhanced template-based fallback that mimics AI behavior
 * 
 * PERFORMANCE: OpenAI and Anthropic run in parallel for speed. Aggressive timeouts to fail fast.
 */
async function generateCompletion({ prompt, temperature = 0.5, maxTokens = 1500, type = 'text', context = {} }) {
  ...
  // Try OpenAI and Anthropic in PARALLEL with 8s timeout each (faster than sequential)
  const openai = getOpenAI();
  const anthropic = getAnthropic();
  
  // Promise that races OpenAI and Anthropic
  const premiumProvidersPromise = Promise.race([
    // OpenAI
    (async () => {
  ...
  try {
    ...
    return await Promise.race([premiumProvidersPromise, createTimeout(12000)]);
  } catch (err) {
    // If premium providers fail, try Hugging Face as fallback with shorter timeout
    logger.info('premium_providers_failed, trying huggingface', { message: err.message });
