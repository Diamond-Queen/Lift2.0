 ...
 * 1. Check which providers are actually configured (have API keys)
 * 2. If premium providers available: run in parallel (8s timeout)
 * 3. Otherwise: use free Hugging Face (5s timeout)
 * 4. Final fallback: template-based generation (always works)
 * 
 * PERFORMANCE: Respects available resources. Skips unconfigured providers.
 */
async function generateCompletion({ prompt, temperature = 0.5, maxTokens = 1500, type = 'text', context = {} }) {
  ...
  const hasAnthropic = !!getAnthropic();
  const hasHuggingFace = !!getHuggingFace();
  
  logger.info('ai_provider_check', { hasOpenAI, hasAnthropic, hasHuggingFace });
  
  // If premium providers (OpenAI/Anthropic) are available, try them in PARALLEL
  if (hasOpenAI || hasAnthropic) {
        ...
        (async () => {
          try {
            ...
            const resp = await Promise.race([
              ...
              createTimeout(6000) // Increased from 8000 to 6000 (race condition - outer race is 12s)
            ]);
            const content = resp.choices?.[0]?.message?.content?.trim();
            if (content) return { provider: 'openai', content };
            throw new Error('No content returned from OpenAI');
          } catch (err) {
            logger.warn('openai_completion_failed', { message: err.message });
            throw err;
          }
        })()
      );
      ...
      premiumAttempts.push(
        (async () => {
          try {
            ...
            logger.warn('anthropic_completion_failed', { message: err.message });
            throw err;
          }
        })()
      );
    ...
    // Only try premium providers if at least one is available
    if (premiumAttempts.length > 0) {
      try {
        // Return whichever premium provider responds first with shorter overall timeout
        const result = await Promise.race([...premiumAttempts, createTimeout(10000)]);
        logger.info('premium_provider_success', { provider: result.provider });
        return result;
      } catch (err) {
        logger.warn('premium_providers_failed, falling back', { message: err.message, availableProviders: { hasOpenAI, hasAnthropic } });
        // Fall through to Hugging Face or template
      }
    }
  }
  
  // Try Hugging Face (free) with shorter timeout
  if (hasHuggingFace) {
    try {
      const hf = getHuggingFace();
      // Updated: Use new router endpoint that replaced api-inference.huggingface.co
      ...
      logger.warn('huggingface_completion_failed', { message: hfErr.message });
      // Try direct fetch call as fallback (SDK might use old endpoint)
      if (process.env.HUGGINGFACE_API_KEY) {
        try {
          ...
          if (directResponse && directResponse[0]?.generated_text) {
            const content = directResponse[0].generated_text.trim();
            if (content) return { provider: 'huggingface', content };
          }
        } catch (directErr) {
          logger.warn('huggingface_direct_fallback_failed', { message: directErr.message });
        }
      }
    }
  }

  // Final fallback: enhanced template-based generation (always works, no API needed)
  logger.info('ai_fallback_to_template', { type: context.type, availableProviders: { hasOpenAI, hasAnthropic, hasHuggingFace }, temperature, maxTokens });
  ...
  throw new Error('All AI providers failed and no fallback template available for type: ' + context.type);
}
