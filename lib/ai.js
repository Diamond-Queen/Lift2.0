...
function buildResumeTemplate({ name, email, phone, address, linkedin, objective, experience, education, skills, certifications }) {
  ...
  const expandObjective = (obj) => {
    ...
    else if (lower.includes('learn') || lower.includes('grow') || lower.includes('develop') ||
             lower.includes('improve') || lower.includes('challenge') || lower.includes('opportunity')) {
    ...
    // Fallback: Intelligent expansion based on input length and complexity
      ...
      // If input is very short, create a structured expansion
      if (text.length < 30) {
        expansion = `${text}. Seeking to leverage expertise and proven capabilities to contribute meaningfully, drive measurable impact, and support organizational success through dedication to excellence.`;
      } else {
        // If input is longer, build on it with complementary language
        expansion = `${text}. Dedicated professional committed to leveraging these strengths to contribute meaningfully, drive operational excellence, and support organizational goals through reliability and collaborative teamwork.`;
      }
    }
