...
function buildResumeTemplate({ name, email, phone, address, linkedin, objective, experience, education, skills, certifications }) {
  ...
  const expandObjective = (obj) => {
    ...
    if (keyTerms.includes('lead') || keyTerms.includes('manage')) {
    ...
    } else if (keyTerms.includes('develop') || keyTerms.includes('engineer') || keyTerms.includes('code')) {
      ...
      expansion = `${text}. Seeking to leverage expertise and experience to contribute meaningfully, drive impact, and support organizational success.`;
    }
