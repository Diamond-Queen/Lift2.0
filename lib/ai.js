...
function buildQuizTemplate({ notes }) {
  ...
  // Helper to build basic distractors
  const makeDistractors = (answer, allCandidates) => {
    ...
    for (const cand of allCandidates) {
      ...
      if (c !== a && c.length > 3 && c.length < 150) {
        distractors.add(c.slice(0, 100));
        if (distractors.size >= 3) break;
      }
    }
    
    // If not enough distractors from candidates, create variants
  ...
  for (let i = 0; i < candidates.length && quiz.length < 10; i++) {
    ...
    if (cleaned.includes(':')) {
      ...
      // Default: ask to identify/recall key fact
      question = `Which of the following is correct?`;
      answer = cleaned.slice(0, 100);
    }

    if (!question || !answer) continue;

    const distractors = makeDistractors(answer, candidates);
