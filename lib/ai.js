...
async function generateCompletion({ prompt, temperature = 0.5, maxTokens = 1500, type = 'text', context = {} }) {
  ...
  if (hasOpenAI || hasAnthropic) {
    ...
    if (premiumAttempts.length > 0) {
      try {
        // Return whichever premium provider responds first
        return await Promise.race([...premiumAttempts, createTimeout(12000)]);
      } catch (err) {
        logger.info('premium_providers_failed, falling back', { message: err.message });
  ...
  logger.info('ai_fallback_to_template', { type: context.type, availableProviders: { hasOpenAI, hasAnthropic, hasHuggingFace } });
