  ...
  providers: [
    CredentialsProvider({
      ...
      async authorize(credentials, req) {
        ...
        // Check if user has a valid subscription or beta access (deny login only if they previously canceled)
        // New users (not onboarded) should always be allowed to login
        if (usePrisma && user.id && user.onboarded) {
          ...
          const betaAccess = await prisma.betaTester.findUnique({
            where: { userId: user.id }
          });
          
          if (!activeSub && !betaAccess) {
            // Account had access but canceled - deny login
            auditLog('auth_account_canceled', user.id, { email: normalized, ip });
            throw new Error('Account no longer has access. Please contact support or re-subscribe.');
          }
        }
