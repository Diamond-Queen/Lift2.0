...
const authOptions = {
  ...
  providers: [
    CredentialsProvider({
      ...
      async authorize(credentials, req) {
        ...
        // Note: Allow all onboarded users to login.
        // Access control (subscription checks) is enforced at the page/API level, not at auth level.
        // This allows users with expired/canceled subscriptions to login and renew.
  ...
  callbacks: {
    async jwt({ token, user }) {
      ...
      // Return token unchanged if no user (refresh case)
      return token;
    },
    async session({ session, token, user }) {
      try {
        if (token) {
          // Ensure session has user object with all required fields from token
          session.user = session.user || {};
          session.user.id = token.id || token.sub;
          session.user.email = token.email || '';
          session.user.name = token.name || '';
          session.user.role = token.role || 'user';
          // Include token ID for fallback reference
          session.user.tokenId = token.sub;
        } else if (user) {
          // Fallback for database sessions
          session.user = session.user || {};
        ...
        if (!session.user.id || !session.user.email) {
          logger.warn('session_missing_required_fields', {
        ...
        // Return empty session on error (will cause redirect to login)
        return { user: {} };
      }
      return session;
    },
