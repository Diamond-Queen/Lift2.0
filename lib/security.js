...
function setSecureHeaders(res) {
  ...
  // Content Security Policy (allow Google Fonts, Tesseract worker, OCR APIs)
  const csp = [
    "default-src 'self'",
    ...
    "connect-src 'self' data: https://api.openai.com https://api.anthropic.com https://www.soundhelix.com https://api.ocr.space https://cdn.jsdelivr.net",
...
setInterval(() => {
  ...
  for (const [key, limiter] of ipLimiters.entries()) {
    if (limiter.blockEndTime && now > limiter.blockEndTime + staleThreshold) {
      ipLimiters.delete(key);
    } else if (!limiter.blockEndTime && now > limiter.resetTime + staleThreshold) {
      ipLimiters.delete(key);
    }
  }
  
  // Clean up old user limiters
  for (const [key, limiter] of userLimiters.entries()) {
    if (now > limiter.resetTime + staleThreshold) {
...
function blockIp(ip, reason = 'blocked', durationMs = 15 * 60 * 1000) {
  const n = normalizeIp(ip);
  const until = Date.now() + durationMs;
  blockedIps.set(n, { until, reason });
  auditLog('ip_blocked', null, { ip, reason, durationMinutes: Math.round(durationMs / 60000), until }, 'warning');
}
...
function incrementSuspicion(ip, reason) {
  ...
  console.warn(`[SECURITY-FIREWALL] Suspicion increment for ${ip} (${reason}): count=${count}`);
  return count;
}
...
function trackIpRateLimit(ip, endpoint = 'global') {
  ...
  if (limiter.blocked && limiter.blockEndTime && Date.now() < limiter.blockEndTime) {
    return { allowed: false, count: limiter.count, limit: 0, reason: 'temporarily_blocked', blockUntil: new Date(limiter.blockEndTime).toISOString() };
  }
  if (limiter.cooldownUntil && Date.now() < limiter.cooldownUntil) {
    // Soft endpoints return a throttle hint instead of hard block
    return { allowed: false, count: limiter.count, limit: 0, reason: 'throttled', retryAfter: Math.ceil((limiter.cooldownUntil - Date.now()) / 1000), blockUntil: new Date(limiter.cooldownUntil).toISOString() };
  }
  ...
  if (limiter.count > effectiveLimit) {
    ...
    limiter.blockEndTime = Date.now() + 120000; // Block for 2 minutes (reduced from 5)
    ...
    return { allowed: false, count: limiter.count, limit: baseLimit, reason: 'rate_limit_exceeded', blockUntil: new Date(limiter.blockEndTime).toISOString() };
  }
  ...
  for (const pattern of sqlInjectionPatterns) {
    if (pattern.test(bodyString)) {
      ...
      if (!isNonBlockingPath) {
        ...
        // Increased threshold from 3 to 5 before blocking
        if (count >= 5) {
          blockIp(nIp, 'sql_injection_detected', 30 * 60 * 1000); // 30 min block for serious threat
  ...
  for (const pattern of xssPatterns) {
    if (pattern.test(bodyString)) {
      ...
      if (!isNonBlockingPath) {
        ...
        // Increased threshold from 3 to 5 before blocking
        if (count >= 5) {
          blockIp(nIp, 'xss_detected', 30 * 60 * 1000); // 30 min block for serious threat
...
async function deliverSecurityAlert(logEntry) {
  ...
  try {
    ...
    // Best-effort delivery with simplified retry logic
    const maxAttempts = 2;
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
      try {
        ...
        const fetchPromise = fetch(webhook, {
          method: 'POST',
        ...
        if (!res.ok) {
          ...
          if (res.status >= 500 && attempt < maxAttempts) {
            await new Promise(r => setTimeout(r, 500 * attempt)); // Shorter backoff
            continue;
          }
          // For client errors (4xx), stop retrying
          break;
        }
        ...
        const isTimeout = err.message === 'Webhook timeout';
        const isNetwork = err.name === 'TypeError' || err.code === 'ECONNREFUSED';
        
        if (isTimeout || isNetwork) {
          console.warn(`[AUDIT-WEBHOOK-WARN] Attempt ${attempt} failed (${isTimeout ? 'timeout' : 'network'})`, { webhook, message: err.message });
          if (attempt < maxAttempts) {
            await new Promise(r => setTimeout(r, 500 * attempt)); // Wait before retry
            continue;
          }
        } else {
          console.error('[AUDIT-WEBHOOK-ERROR] Unexpected error', { webhook, attempt, message: err.message });
        }
      }
    }
  } catch (err) {
    console.error('[AUDIT-WEBHOOK-ERROR] Failed to send alert', err.message);
  }
}
...
function listBlockedIps() {
  ...
  const blocked = [];
  for (const [ip, entry] of blockedIps.entries()) {
    // Only return IPs that are still blocked (not expired)
    if (now < entry.until) {
      blocked.push({
        ip,
        reason: entry.reason,
        ...
        remainingMinutes: Math.ceil((entry.until - now) / 60000)
      });
    }
  }
  return blocked;
