...
function setSecureHeaders(res) {
  ...
  const csp = [
    ...
    "font-src 'self'",
    "connect-src 'self' https://api.openai.com https://www.soundhelix.com",
...
function validateRequest(req) {
  ...
  for (const pattern of sqlInjectionPatterns) {
    if (pattern.test(bodyString)) {
      console.error(`[SECURITY-FIREWALL] SQL injection attempt detected from ${clientIp}`);
  ...
  for (const pattern of xssPatterns) {
    if (pattern.test(bodyString)) {
      console.error(`[SECURITY-FIREWALL] XSS attempt detected from ${clientIp}`);
